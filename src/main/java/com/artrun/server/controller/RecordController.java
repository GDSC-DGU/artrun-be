...
public class RecordController {
    ...
    public ResponseEntity<ApiResponse<RecordDetailResponse>> getRecord(
        ...
        return ResponseEntity.ok(ApiResponse.ok(recordService.getRecord(userDetails.getUserId(), recordId)));
    }

    @Operation(summary = "SNS 공유 카드 재생성")
    @PostMapping("/{recordId}/share-card")
    public ResponseEntity<ApiResponse<Map<String, String>>> regenerateShareCard(
            @AuthenticationPrincipal CustomUserDetails userDetails,
            @PathVariable String recordId) {
        String imageUrl = recordService.regenerateShareCard(userDetails.getUserId(), recordId);
        return ResponseEntity.ok(ApiResponse.ok("공유 카드가 재생성되었습니다.",
                Map.of("imageUrl", imageUrl != null ? imageUrl : "")));
    }
